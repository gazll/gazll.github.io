...
function newLink(link, origin, now) {
  const row = {
    ...
    category: categoryOf(link.name || link.code),
    origin,
    sourceIds: [],
    parents: [],
    path: '',
...
export function projectCatalog(catalog, sealedAt = new Date().toISOString()) {
  ...
  const links = catalog.links
    ...
    .map((row) => ({
      ...
      ...(row.category && row.category !== 'movie' ? { category: row.category } : {}),
      status: row.status,
...
async function main() {
  ...
  if (command === 'categorize') {
    ...
    catalog.links.forEach((row) => {
      ...
      const to = categoryOf(row.name);
      before[from]++;
      after[to]++;
      if (from !== to) {
        moved.push({ from, to, name: row.name });
