...
`INTEGRATED` (nghiên cứu xong, nguồn đã thẩm định), nhưng các record vẫn còn
ô `[ ]`. Đọc kỹ thì chúng là **ba loại khác nhau**, và chỉ loại đầu là việc:

| | Bao nhiêu | Là gì | Ở đây |
|---|---:|---|---|
| **A. Đề xuất sửa nội dung** | 235 | Câu live còn nói quá, lỗi thời, thiếu ràng buộc, hoặc thiếu link sang chủ sở hữu. Đây là việc biên tập thật, chưa áp dụng vào `public/data/`. | Phần A, theo unit |
| **B. Câu hỏi mở / falsifier** | 141 | *"Which broker/version is the deployment targeting?"* — tham số phụ thuộc môi trường triển khai. Cố ý để mở (`DEPLOYMENT INPUTS OPEN`), không phải việc chưa xong. | Phần B, gom một chỗ để nhìn lướt |
| C. Ô gate quy trình | 32 | *"Content changes integrated into public/data"* chưa tick dù status đã `INTEGRATED` — sổ sách lệch, không phải nội dung. | Bỏ qua; tick trong record khi tiện |

Xoá file này khi phần A hết ô trống. Lúc đó `docs/research/` chỉ còn giá trị
tra cứu nguồn (605 dòng source ledger, ~1.000 URL đã thẩm định, ngày review
từng nguồn) — quyết định xoá hay giữ thư mục để sau đó.

## Rà thế nào

Với mỗi dòng ở phần A, chọn một trong ba: **làm** (sửa theo
`docs/content-playbook.md`, tick `[x]` ở đây và ở record gốc), **bỏ** (gạch
`~~…~~` kèm lý do: lỗi thời / đã có ở topic khác / đủ rồi), hoặc **để sau**.

Không rà tuần tự. Chia theo loại việc, vì chi phí và rủi ro khác nhau:

| Loại | Nhận ra bằng | ~Số | Rà thế nào |
|---|---|---:|---|
| **Sửa cho đúng** | *Replace / Remove / Mark / Qualify / Correct / Separate / Keep … but* | 98 | **Trước.** Đây là nơi có claim đang hiển thị mà nguồn không đỡ được. Mở câu live, đối chiếu bảng *Sources* trong record; đa số là sửa một câu, giữ nguyên `id`. |
| **Thêm** | *Add / Include / Provide / Extend* | 84 | Đắt nhất và dễ làm mục dài ra vô ích. Chỉ làm khi câu trả lời hiện tại thật sự không trả lời được câu hỏi phỏng vấn; còn lại gạch với lý do "đủ rồi". |
| **Liên kết** | *Link / Cross-ref / Merge* | 3 | Rẻ: thêm `(item-id)` theo rule "one owner per mechanism". |
| Khác | — | 50 | Đọc từng dòng. |

Thứ tự unit nếu muốn kết quả sớm: **27, 10, 13, 20, 28** (gateway, overload,
security, observability, lock) — nhiều mục *sửa cho đúng* nhất, và là các topic
hay bị hỏi "bạn sẽ làm gì bây giờ"; rồi **19** (trình bày DSA), **26, 14, 21**;
case study để cuối vì body là bài lưu trữ, chỉ *guide* mới sửa được.

Ba việc **không** làm trong đợt này: không xoá item (`id` là khoá Sheet),
không điền `reviewed_at` cho unit chưa đọc lại, không sửa body case study
ngoài việc bỏ link chết.

Đã làm trước một mục (2026-09-12, `11ae645`): topic 27 *"Replace the local
Spring Cloud Gateway 5.0.2 wording…"* — đã pin 5.0.3 / 2025.1.3 theo Maven Central.

# A. Đề xuất sửa nội dung — 235 mục

## Topics — Study Track — 200 mục
...
### 09-distributed-tx-fintech — 5 mục
...
### 10-system-design-rate-limit — 10 mục
...
### 14-devops-k8s-best-practices — 11 mục
...
### 21-linux-production-debug — 10 mục
...
### 27-api-gateway-identity-edge — 12 mục
...
### 28-distributed-lock-lease — 10 mục
...
### 13-how-discord-stores-trillions-of-messages — 8 mục
...
### 14-small-business-cloud-cost-shock — 8 mục
...
### 15-transactional-outbox-order-workflow — 9 mục
...
# B. Câu hỏi mở / falsifier — 141 mục (không phải việc)

Giữ nguyên văn để biết mỗi unit đang giả định gì. Một dòng chỉ "đóng" khi hoặc
nội dung đã nêu rõ giả định đó ("assume X; if not, Y"), hoặc bạn quyết định
ghi nó là tham số triển khai và thôi. Không có deadline.
...
### 14-devops-k8s-best-practices — 7
...
### 16-project-concurrency-whiteboard — 7
...
### 27-api-gateway-identity-edge — 10
...
### 10-xac-thuc-va-phan-quyen-trong-microservices — 6
...
### 11-how-to-handle-hot-deals-at-the-peak-time — 6
...
### 12-duplicate-booking-race-condition — 6
...
### 13-how-discord-stores-trillions-of-messages — 6
...
### 14-small-business-cloud-cost-shock — 6
...
### 15-transactional-outbox-order-workflow — 6
