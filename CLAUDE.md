...
vendor/mermaid-11.16.1/  pinned upstream build; version lives in the directory name
  data/
    ...
    topics/NN-slug.json     complete English base, one file per topic (one file per manifest row;
                         `node tools/validate-content.mjs` prints the live item count — do not
                         restate it here, it drifts with every content batch)
    topics/NN-slug.vi.json  complete Vietnamese companion, same shape and item IDs
    content-index.json   GENERATED by tools/build-content-index.mjs: every item id, its
                         question text in both languages, and each topic's track item ids.
                         Loads with the manifest so the progress ring, cross-reference
                         links and deep links work before a topic's own file arrives
    content-reviews.json reviewed_at + target_versions per version-bound claim
    ...
    system-design/research.js   the one ES module under data/: bilingual "Engineering
                         deep dives" packs plus the design→pack assignment map. It is code
                         only because the prose carries links the view renders; the origins
                         it may cite are REFERENCE_ORIGINS in lib/constants.js
...
- **The progress ring counts what the Study Track browses, not what `data/`
  ...
  them from it rather than from here, because a written-down count goes stale
  the next time content lands (it already did, twice in one day). Two
...
- **An archived body's own links are governed too, just by different rules.**
  ...
  case-study body carries the original article's links, and those had no check
  at all — images could not hotlink, `<a href>` was free. Two things a
  preserved body must not keep: a **recruitment funnel** (four articles shipped
  a publisher's 2020 "join us" CTA, addressed to our reader as if we were
  hiring) and a **`http://` link** — `upgrade-insecure-requests` rewrites it, so
  a host without TLS becomes a link that cannot succeed. Both are asserted in
  `tests/case-studies.test.mjs`. What a body *should* keep is the original's
  own citations, even to a blog or a video: that is the article's provenance,
  not ours. When such a source is dead, unlink it and leave the URL as `<code>`
  — the citation survives, the broken promise does not. Same reason `alt` text
  may not read "Photo by …": the archive does not store contributor names.

- **Outbound links have exactly one allowlist owner: `lib/constants.js`.**
  Three sets, three different promises, and they must not be merged.
  `PUBLISHER_ORIGINS` is a credit — "the original was published here" — and is
  what a case study or production case links. `PROMPT_ORIGINS` is a discussion
  thread that supplied a *question* (a blueprint's `source_url`); it is never an
  authority for a claim, so it must never appear in the publisher set, and a
  test asserts the two do not overlap. `REFERENCE_ORIGINS` is primary vendor
  docs, specs and RFCs cited by research packs and journal entries — per
  "Content standards" this list grows when a better source needs it.
  `originGuard(origins, fallback)` builds the check; `'#'` keeps the anchor and
  `''` drops the link. **Never re-declare a set or re-implement the guard in a
  view, a tool or a test.** Five copies had already drifted: the views allowed
  `blog.cloudmentor.pro` while `validate-content.mjs` silently did not, and
  nothing at all validated a blueprint's `source_url`. Both now import the same
  arrays, and a blueprint with a `source_url` must also carry `en.source_label`
  and `vi.source_label` naming what the reader is being sent to.
...
header that appears in the middle throws `Header không khớp` rather than
shifting every stored column one to the right.
