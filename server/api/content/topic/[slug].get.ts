...
import { progressIndex, referencedIds, slimQuestions } from '../../../utils/content-index.js';
...
export default defineEventHandler(async event => {
  ...
  const [manifest, meta, reviews, systemDesign, contentIndex] = await Promise.all([
  ...
  const ids = referencedIds({ en, vi });
  return {
    rows, row, meta: meta.topics[String(row.n)], topicMeta: meta.topics, reviews, en, vi, stem,
    sourceOwners, crossRefs: slimQuestions(contentIndex, ids), progressIndex: progressIndex(contentIndex)
  };
});
