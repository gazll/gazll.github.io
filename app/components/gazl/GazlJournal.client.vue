...
const form = reactive<any>({ id: '', name: '', role: '', date: '', result: 'pending', stack: '', references: [], questions: [] });
const formError = ref('');
...
<template>
  <div id="ivRoot" class="iv-root">
    ...
    <div class="toolbar"><span class="sectioncount">{{ companies.length }} {{ companies.length === 1 ? labels.entry : labels.entries }}<span v-if="editable"> ({{ ownCount }} {{ labels.mine }} · {{ seedCount }} {{ labels.samples }})</span> · {{ questionCount }} {{ questionCount === 1 ? labels.question : labels.questions }} <span v-if="!editable" class="ro">· {{ labels.readOnly }}</span></span><div class="tb-actions"><button v-if="editable" id="ivAdd" class="btn-primary" type="button" @click="resetForm()">+ {{ labels.addCompany }}</button><span v-else class="hint">{{ $auth.enabled ? labels.signIn : labels.noBackend }}</span></div></div>

    <div v-if="!loading && !companies.length" class="page"><p>{{ labels.noEntries }}</p></div>
    <article v-for="company in companies" :key="company.id" class="company" :class="{ 'is-seed': !company.own }">
      <div class="company-head"><h2>{{ company.name }}</h2><span v-if="!company.own" class="seed-badge">{{ labels.sample }}</span><span v-if="kindLabels[company.kind]" class="entry-kind">{{ kindLabels[company.kind] }}</span><span v-if="company.result" class="result" :class="`result-${company.result}`">{{ resultLabels[company.result] || company.result }}</span><div v-if="editable" class="co-actions"><template v-if="company.own"><button class="btn-ghost sm" type="button" @click="resetForm(company)">{{ labels.edit }}</button><button class="btn-ghost sm danger" type="button" :disabled="busy === company.id" @click="remove(company)">{{ labels.remove }}</button></template><button v-else class="btn-ghost sm" type="button" :disabled="busy === company.id" @click="importSeed(company)">{{ busy === company.id ? labels.saving : labels.saveToJournal }}</button></div></div>
      <p v-if="company.role || company.date" class="company-meta">{{ [company.role, company.date].filter(Boolean).join(' · ') }}</p>
