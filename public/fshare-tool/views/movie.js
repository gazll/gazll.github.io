...
import {
  MOVIE_DB_URL, groupByTitle, normalizeMovieDatabase, searchMovieLinks, sourceName
...
function makeGroupHead(group) {
  ...
  head.className = 'movie-group-head' + (group.links.length > 1 ? ' multi' : '');
  head.textContent = group.name;
  const count = document.createElement('span');
  count.textContent = group.links.length > 1 ? `${group.links.length} links · same title` : '';
...
function renderResults() {
  ...
  const kind = $('movieKindSelect').value || 'all';
  const sourceId = $('movieSourceSelect').value || 'all';
  const showDead = $('movieShowDead').checked;
  const matches = searchMovieLinks(movie.database.links, query, { kind, sourceId })
    .filter((row) => showDead || currentStatus(row).status === 'live');
  const groups = groupByTitle(matches);
  ...
  if (!groups.length) {
    ...
    setText('movieResultCount', `0 of ${number(movie.database.links.length)} links`);
    renderControls();
    return;
  }
  ...
  setText('movieResultCount', `${number(matches.length)} link${matches.length === 1 ? '' : 's'} in ${number(groups.length)} title${groups.length === 1 ? '' : 's'}${suffix} · ${number(movie.selected.size)} selected`);
  renderControls();
}
...
function wireMovieEvents() {
  ...
  ['movieSourceSelect', 'movieKindSelect', 'movieShowDead'].forEach((id) => $(id).addEventListener('change', renderResults));
  $('movieClearSearch').addEventListener('click', () => {
    $('movieSearchInput').value = '';
    $('movieSourceSelect').value = 'all';
    $('movieKindSelect').value = 'all';
